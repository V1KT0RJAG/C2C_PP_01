...
class Infrared(object):
    ...
    def __init__(self, references: list = None) -> None:
        """Constructor of the class Infrared

        Args:
            references (list, optional): List of floats to use as references. Defaults to [300, 300, 300, 300, 300].
        """
    ...
    def read_digital(self) -> list:
        ...
        """
        analog = np.array(self.read_analog(), dtype=float)
        references = np.array(self._references, dtype=float)
        digital = np.where(analog < references, 1, 0)
        return list(digital)
    ...
    def save_references(self, filename: str) -> None:
    # """Speichert die Referenzwerte in einer JSON-Datei."""
        with open(filename, 'w') as f:
            json.dump(self._references.tolist(), f)  # <- Umwandlung in Liste
        print(f"Referenzwerte gespeichert in {filename}")

    def load_references(self, filename: str) -> None:
        """Lädt die Referenzwerte aus einer JSON-Datei."""
        with open(filename, 'r') as f:
            self._references = json.load(f)
        print(f"Referenzwerte geladen aus {filename}")
